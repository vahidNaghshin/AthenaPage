...
try:
    from langchain.chat_models import init_model  # type: ignore
except ImportError:
    init_model = None
...
def create_aws_session(aws_profile: Optional[str]) -> boto3.Session:
    try:
        ...
        print(f"AWS profile '{aws_profile}' was not found. Falling back to default AWS credential chain.")
...
async def set_ext_context(payload: dict = Body(default_factory=dict)):
    ...
    LAST_PAGE_CONTEXT["page_title"] = normalize_str(payload.get("page_title"), "Unknown")
...
async def on_start():
    ...
    cl.user_session.set("system_prompt", f"""
...
async def on_message(message: cl.Message):
    ...
    # answer = await call_your_llm(system_prompt, history)
    ...
    if not llm_chain:
        await cl.Message(content="Model is not initialized. Start a new chat session.").send()
        return

    try:
        if LLMChain is not None and hasattr(llm_chain, "apredict"):
            text = await llm_chain.apredict(
        ...
        await cl.Message(content="AWS credentials are missing. Please configure AWS auth.").send()
        return
